...
import { Expense, MONTH_MAP, MonthReportData, User } from '@/app/global'
...
// Dynamically import the PDF download link component (client-side only)
const ReportViewer = dynamic(
  () => import('./react-pdf/ReportViewer.client'),
  { ssr: false, loading: () => <p>Loading download link…</p> }
)
...
export default function DesktopView()  {
  ...
  // Prepare report data inline (always up-to-date)
  ...
  for (const exp of losses) {
    ...
    categoryTotalsForExpenses[category] = (categoryTotalsForExpenses[category] || 0) + amount;
  }

  const categoryTotalsForProfit: Record<string, number> = {};
  for (const exp of gains) {
    const { category, amount } = exp;
    categoryTotalsForProfit[category] = (categoryTotalsForProfit[category] || 0) + amount;
  }

  // Compute most frequent expense category
  const categoryCounts: Record<string, number> = {};
  losses.forEach(exp => {
    categoryCounts[exp.category] = (categoryCounts[exp.category] || 0) + 1;
  });

  // Biggest expense category by total amount
  const biggestExpenseCategory =
    Object.keys(categoryTotalsForExpenses).reduce(
      (a, b) => (categoryTotalsForExpenses[a] > categoryTotalsForExpenses[b] ? a : b),
  ...
  const highestOccCategory =
    Object.keys(categoryCounts).reduce(
      (a, b) => (categoryCounts[a] > categoryCounts[b] ? a : b),
      Object.keys(categoryCounts)[0] || ''
    );

    const biggestExpenseAmount = categoryTotalsForExpenses[biggestExpenseCategory] || 0;
    const biggestExpensePercentage = totalLost > 0
      ? (biggestExpenseAmount / totalLost) * 100
      : 0;
      ...
      <div className="w-full max-w-screen-lg bg-white rounded-lg shadow-sm p-15 flex-grow">
        <div className="flex items-center mb-6">
          <Image
            ...
            className="mr-4"
          />
          <h1 className="text-3xl font-extrabold text-gray-900">Pocket Panda</h1>
        </div>
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">{MONTH_MAP[reportData.month]} {reportData.year} Report</h1>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 bg-gray-200">
              <tbody className="bg-gray-200 divide-y divide-gray-200">
                ...
                <tr>
                  ...
                  <td className="px-4 py-2 text-right font-bold">${reportData.totalGained.toFixed(2)}</td>
                </tr>
                {/* Category totals for losses grouped by category */}
                {Object.entries(categoryTotalsForExpenses).map(([category, amount]) => (
                  <tr key={category}>
                    <td className="px-4 py-2">{category}</td>
                    <td className="px-4 py-2 text-right">${amount.toFixed(2)}</td>
                  </tr>
                ...
                <tr>
                  ...
                  <td className="px-4 py-2 text-right font-bold">${reportData.totalLost.toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="space-y-4 mb-4 m-4">
          <h2 className="text-2xl font-semibold border-b-2 border-gray-200 pb-2">Spending Breakdown</h2>
          <div className="mt-6 w-full flex space-x-4">
            <div className="w-full md:w-1/2 aspect-square bg-white p-4 rounded-lg shadow-sm flex items-center justify-center">
              <PieChartFlex data={losses} groupByCategory={true} labelOn={false}/>
            </div>
            ...
            <ul className="mt-4 md:mt-0 w-full md:w-1/2 bg-gray-100 rounded-lg p-4 space-y-3">
              <li className="list-disc list-inside text-gray-800">
                <strong>Total spending:</strong> ${reportData.totalLost.toFixed(2)}
              </li>
              <li className="list-disc list-inside text-gray-800">
                <strong>Largest Expense Category:</strong> {biggestExpenseCategory} <span className="text-gray-600">(${categoryTotalsForExpenses[biggestExpenseCategory]?.toFixed(2)})</span>
              </li>
              <li className="list-disc list-inside text-gray-800">
                <strong>Largest Expense Category:</strong> {biggestExpenseCategory} accounts for {biggestExpensePercentage.toFixed(1)}% of total spending
              </li>
              <li className="list-disc list-inside text-gray-800">
                <strong>Biggest Single Purchase:</strong> {biggestPurchaseText}
              </li>
              <li className="list-disc list-inside text-gray-800">
                <strong>Most Frequent Expense Category:</strong> {highestOccCategory} <span className="text-gray-600">({categoryCounts[highestOccCategory] || 0} transactions)</span>
              </li>
              <li className="list-disc list-inside text-gray-800">
                <strong>Total Transactions:</strong> {losses.length} transactions
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="space-y-4 mt-6">
          <h2 className="text-2xl font-semibold border-b-2 border-gray-200 pb-2">Detailed Expenses</h2>
          {loading ? <p className="text-center text-gray-500">Loading...</p> : <ExpenseSheet expenses={losses} />}
        </div>

        <div className="space-y-4 mt-6">
          <h2 className="text-2xl font-semibold border-b-2 border-gray-200 pb-2">Detailed Gains</h2>
          {loading ? <p className="text-center text-gray-500">Loading...</p> : <ExpenseSheet expenses={gains} />}
        </div>
      </div>
    </div>
  );
